...
void Application::Alert(const char* status, const char* message, const char* emotion, const std::string_view& sound) {
    ...
    if (!sound.empty()) {
        //ResetDecoder(); fixme: p3 file is not supported yet
        //PlaySound(sound); 
...
void Application::Start() {
    ...
    auto codec = board.GetAudioCodec();
    opus_decoder_ = std::make_unique<DummyDecoderWrapper>(codec->output_sample_rate(), 1, OPUS_FRAME_DURATION_MS);
    opus_encoder_ = std::make_unique<DummyEncoderWrapper>(16000, 1, OPUS_FRAME_DURATION_MS);
...
void Application::SetDecodeSampleRate(int sample_rate, int frame_duration) {
    ...
    opus_decoder_.reset();
    opus_decoder_ = std::make_unique<DummyDecoderWrapper>(sample_rate, 1, frame_duration);
